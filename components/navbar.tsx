...
import { Menu, X, Circle, Triangle, Square, Camera } from "lucide-react";
  ...
  return (
    <nav className="fixed w-full z-50 bg-black/90 backdrop-blur-sm border-b border-pink-500/20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
        ...
        {/* Center - Squid Game Logo */}
        <div className="absolute left-1/2 transform -translate-x-1/2">
          <img src="/logo/squid_game-logo.png" alt="Squid Game Logo" className="h-12" />
        </div>
        ...
        <div className="px-4 pt-2 pb-3 space-y-1">
          {menuItems.map((item) => (
            ...
            >
              ...
              >
                <motion.div
                  className="absolute inset-0 bg-[#FF2D55] opacity-0 group-hover:opacity-10 transition-opacity duration-300"
                />
                <item.icon className="h-4 w-4 mr-2 text-[#FF2D55] group-hover:animate-pulse" />
